...
try {
    ...
    runDdlSafely($db, "ALTER TABLE student_profiles ADD COLUMN IF NOT EXISTS previous_school VARCHAR(255)");
    echo "✓ Added previous_school column\n";
    
    // Add nationality column
    runDdlSafely($db, "ALTER TABLE student_profiles ADD COLUMN IF NOT EXISTS nationality VARCHAR(100)");
    echo "✓ Added nationality column\n";
    
    // Add religion column
    runDdlSafely($db, "ALTER TABLE student_profiles ADD COLUMN IF NOT EXISTS religion VARCHAR(100)");
    echo "✓ Added religion column\n";
    
    // Add parent_name column (for backward compatibility)
    runDdlSafely($db, "ALTER TABLE student_profiles ADD COLUMN IF NOT EXISTS parent_name VARCHAR(100)");
    echo "✓ Added parent_name column\n";
    
    // Add parent_phone column (for backward compatibility)
    runDdlSafely($db, "ALTER TABLE student_profiles ADD COLUMN IF NOT EXISTS parent_phone VARCHAR(20)");
    echo "✓ Added parent_phone column\n";
    
    // Add parent_email column (for backward compatibility)
    runDdlSafely($db, "ALTER TABLE student_profiles ADD COLUMN IF NOT EXISTS parent_email VARCHAR(100)");
    echo "✓ Added parent_email column\n";
    
    // Add admission_number column
    runDdlSafely($db, "ALTER TABLE student_profiles ADD COLUMN IF NOT EXISTS admission_number VARCHAR(50)");
    echo "✓ Added admission_number column\n";
    
    // Add class_id column for direct class reference
    runDdlSafely($db, "ALTER TABLE student_profiles ADD COLUMN IF NOT EXISTS class_id INT");
    echo "✓ Added class_id column\n";
    
    // Add foreign key constraint for class_id
    try {
        runDdlSafely($db, "ALTER TABLE student_profiles ADD CONSTRAINT IF NOT EXISTS fk_student_profiles_class 
