...
$min_lates      = isset($_GET['min_lates']) && $_GET['min_lates'] !== '' ? max(1, (int)$_GET['min_lates']) : 1;
...
// ---- Per-student punctuality data (only students who have shown up at all) ----
$report_query = "
    SELECT
        u.id AS student_id,
        ...
        SUM(CASE WHEN a.status = 'present' THEN 1 ELSE 0 END) AS ontime_days,
    ...
    HAVING late_days >= :min_lates
    ORDER BY late_days DESC, attended_days DESC, student_name ASC
";
$report_params = array_merge($base_params, [':min_lates' => $min_lates]);
...
$summary_query = "
    SELECT
        ...
        SUM(CASE WHEN a.status = 'present' THEN 1 ELSE 0 END) AS total_ontime,
...
$total_ontime   = (int)($summary['total_ontime'] ?? 0);
$total_late     = (int)($summary['total_late'] ?? 0);
$students_late  = (int)($summary['students_late'] ?? 0);
$punctuality_rate = $total_attended > 0 ? ($total_ontime / $total_attended) * 100 : 0;

// ---- Lateness by class (bar chart) ----
$class_query = "
...
// ---- Daily lateness trend (line chart) ----
$trend_query = "
    SELECT a.date,
           SUM(CASE WHEN a.status = 'late' THEN 1 ELSE 0 END) AS late_days,
           SUM(CASE WHEN a.status = 'present' THEN 1 ELSE 0 END) AS ontime_days
...
if (isset($_GET['export']) && $_GET['export'] === 'csv') {
    ...
    header('Content-Disposition: attachment; filename="punctuality_report_' . date('Ymd_His') . '.csv"');
    $out = fopen('php://output', 'w');
    fputcsv($out, ['Punctuality Report']);
    fputcsv($out, ['Class', $selected_class_name]);
    fputcsv($out, ['Period', $from_date . ' to ' . $to_date]);
    fputcsv($out, ['Generated', date('Y-m-d H:i')]);
    fputcsv($out, []);
    fputcsv($out, ['Rank', 'Student Name', 'Student ID', 'Class', 'Days Attended', 'On-Time', 'Lates', 'Punctuality Rate (%)']);
    $rank = 1;
    foreach ($report_data as $row) {
        $attended = (int)$row['attended_days'];
        $rate = $attended > 0 ? round(($row['ontime_days'] / $attended) * 100, 1) : 0;
        fputcsv($out, [
            $rank++,
            $row['student_name'],
            $row['roll_number'] ?? 'N/A',
        ...
        <main class="p-6 lg:p-8 flex-1">
            <div class="w-full">
                ...
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                    <div>
                        <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Punctuality Report</h1>
                        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Monitor student punctuality and tardiness patterns across classes and time.</p>
                    </div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-150 dark:border-gray-700 p-6 mb-6">
                    ...
                    <form method="GET" class="grid grid-cols-1 md:grid-cols-5 gap-4">
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Min. Lates</label>
                            <input type="number" name="min_lates" min="1" value="<?php echo (int)$min_lates; ?>"
                                   class="w-full border border-gray-300 dark:border-gray-650 rounded-lg px-3 py-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 transition">
                        </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-150 dark:border-gray-700 p-6 flex items-center justify-between">
                        <div>
                            <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Punctuality Rate</p>
                            <h3 class="text-3xl font-bold text-gray-900 dark:text-white mt-1"><?php echo number_format($punctuality_rate, 1); ?>%</h3>
                        </div>
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-150 dark:border-gray-700 p-6 flex items-center justify-between">
                        <div>
                            <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Students Late</p>
                            <h3 class="text-3xl font-bold text-gray-900 dark:text-white mt-1"><?php echo $students_late; ?></h3>
                        </div>
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-150 dark:border-gray-700 p-6 flex items-center justify-between">
                        <div>
                            <p class="text-sm font-medium text-gray-500 dark:text-gray-400">On-Time Days</p>
                            <h3 class="text-3xl font-bold text-gray-900 dark:text-white mt-1"><?php echo $total_ontime; ?></h3>
                        </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-150 dark:border-gray-700 p-6">
                        <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-1">Lateness Trend</h3>
                        <p class="text-xs text-gray-500 dark:text-gray-400 mb-4">Daily late arrivals over the selected period</p>
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-150 dark:border-gray-700 p-6">
                        <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-1">Lateness by Class</h3>
                        <p class="text-xs text-gray-500 dark:text-gray-400 mb-4">Classes with the most late arrivals in the period</p>
                        <div class="relative" style="height: 260px;">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-150 dark:border-gray-700 overflow-hidden mb-6">
                    <div class="px-6 py-4 border-b border-gray-150 dark:border-gray-700">
                        <h2 class="text-lg font-bold text-gray-900 dark:text-white">Students with Late Arrivals</h2>
                        ...
                        <table class="min-w-full divide-y divide-gray-250 dark:divide-gray-750">
                            <thead class="bg-gray-50 dark:bg-gray-750">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-center text-xs font-semibold text-gray-500 dark:text-gray-300 uppercase tracking-wider">Days Attended</th>
                                    <th class="px-6 py-3 text-center text-xs font-semibold text-gray-500 dark:text-gray-300 uppercase tracking-wider">On-Time</th>
                                    <th class="px-6 py-3 text-center text-xs font-semibold text-gray-500 dark:text-gray-300 uppercase tracking-wider">Lates</th>
                                    <th class="px-6 py-3 text-right text-xs font-semibold text-gray-500 dark:text-gray-300 uppercase tracking-wider">Punctuality Rate</th>
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                ...
                                foreach ($report_data as $student):
                                    $late = (int)$student['late_days'];
                                    $attended = (int)$student['attended_days'];
                                    $rate = $attended > 0 ? ((int)$student['ontime_days'] / $attended) * 100 : 0;
                                    $tardiness = $attended > 0 ? ($late / $attended) * 100 : 0;
                                    if ($tardiness >= 25) {
                                        $status = 'Chronic'; $status_class = 'text-red-800 bg-red-100 dark:bg-red-900/40 dark:text-red-300';
                                    } elseif ($tardiness >= 10) {
                                        $status = 'Needs Attention'; $status_class = 'text-amber-800 bg-amber-100 dark:bg-amber-900/40 dark:text-amber-300';
                                    } else {
                                        $status = 'Mostly Punctual'; $status_class = 'text-green-800 bg-green-100 dark:bg-green-900/40 dark:text-green-300';
                                    }
                                ?>
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-750 transition duration-150">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-600 dark:text-gray-350"><?php echo $attended; ?></td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-center font-semibold text-green-600 dark:text-green-400"><?php echo (int)$student['ontime_days']; ?></td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-center font-bold text-amber-600 dark:text-amber-400"><?php echo $late; ?></td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-center">
                                        <span class="inline-flex px-2.5 py-1 text-xs font-bold rounded-full <?php echo $status_class; ?>"><?php echo $status; ?></span>
                                    </td>
                                </tr>
                                <?php $rank++; endforeach; ?>
                            </tbody>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-150 dark:border-gray-700 p-12 text-center">
                    ...
                    <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-2">No Tardiness Records</h3>
                    <p class="text-gray-500 dark:text-gray-400 max-w-md mx-auto">No late arrivals match the selected filters. Try widening the date range, changing the class, or lowering the minimum lates.</p>
                </div>
                <?php endif; ?>
            </div>
        </main>
...
<div id="print-report" class="print-report-container">
    <div class="print-page">
        ...
        <div class="print-title-banner"><h2>Punctuality Report</h2></div>
        ...
        <div class="print-meta-grid">
            ...
            <div class="print-meta-item">
                <span class="print-meta-label">Punctuality Rate:</span>
                <span class="print-meta-value"><?php echo number_format($punctuality_rate, 1); ?>%</span>
            </div>
            <div class="print-meta-item">
        ...
        <div class="print-section-title">Students with Late Arrivals</div>
        <table class="print-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?php echo (int)$student['ontime_days']; ?></td>
                    <td class="pct-cell"><?php echo (int)$student['late_days']; ?></td>
...
<style>
    ...
    .print-title-banner { text-align: center; background: linear-gradient(135deg,#1e3a5f,#2563eb); color: #fff; padding: 7px 20px; border-radius: 5px; margin-bottom: 12px; }
    ...
    .print-section-title { font-size: 11px; font-weight: 700; color: #1e3a5f; text-transform: uppercase; letter-spacing: 0.6px; padding: 5px 10px; background: #eef2f7; border-left: 4px solid #2563eb; border-radius: 0 4px 4px 0; margin-bottom: 8px; }
    .print-table { width: 100%; border-collapse: collapse; margin-bottom: 14px; font-size: 10px; }
    .print-table thead th { background: linear-gradient(135deg,#1e3a5f,#2563eb); color: #fff; padding: 7px 8px; text-align: center; font-weight: 600; font-size: 9px; text-transform: uppercase; letter-spacing: 0.4px; border: 1px solid #1a3455; }
    ...
    if (classCanvas) {
        new Chart(classCanvas.getContext('2d'), {
            ...
            data: {
                ...
                datasets: [{
                    label: 'Late Arrivals',
                    data: <?php echo json_encode(array_map(fn($r) => (int)$r['late_days'], $class_data)); ?>,
                    backgroundColor: 'rgba(245, 158, 11, 0.85)',
                    borderColor: 'rgb(245, 158, 11)', borderWidth: 1, borderRadius: 5
                }]
            },
            options: {
                responsive: true, maintainAspectRatio: false,
