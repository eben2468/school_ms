<?php
session_start();
if (!isset($_SESSION['user_id']) || !in_array($_SESSION['role'], ['super_admin', 'school_admin', 'teacher', 'staff'])) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($item_id && $quantity_requested && $purpose) {
        try {
            ...
            $check_query = "SELECT item_name, quantity_available FROM inventory_items WHERE id = :item_id AND status = 'active'";
            ...
            $query = "INSERT INTO inventory_requests (item_id, requested_by, quantity_requested, purpose, request_date, required_date, notes, status, created_at)
                     VALUES (:item_id, :requested_by, :quantity_requested, :purpose, CURDATE(), :required_date, :notes, 'pending', NOW())";
            ...
            $success = "Inventory request created successfully! Request ID: #" . $request_id;
        } catch (Exception $e) {
...
// Get inventory items
// Handle different schema versions - check if category column exists
try {
    // First try with category column
    $items_query = "SELECT id, item_name, item_code, category, quantity_available, unit_price, location
    ...
    // If category column doesn't exist, try with category_id and join
    if (strpos($e->getMessage(), 'category') !== false) {
        $items_query = "SELECT ii.id, ii.item_name, ii.item_code,
                        COALESCE(ic.name, 'General') as category,
                        ii.quantity_available, ii.unit_price, ii.location
                        FROM inventory_items ii
                        LEFT JOIN inventory_categories ic ON ii.category_id = ic.id
                        WHERE ii.status = 'active' AND ii.quantity_available > 0
                    ...
                    <div class="p-6">
                        <form method="POST" class="space-y-6">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div id="item-details" class="md:col-span-2 hidden bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                                    <h4 class="font-medium text-gray-900 dark:text-white mb-2">Item Details</h4>
                                ...
                                <div>
                                    ...
                                    <input type="number" id="quantity_requested" name="quantity_requested" min="1" required
                                           class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white">
                                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Maximum available: <span id="max-quantity">-</span></p>
                                </div>
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <select id="purpose" name="purpose" required
                                        ...
                                        <option value="classroom_use">Classroom Use</option>
                                        <option value="laboratory_experiment">Laboratory Experiment</option>
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <textarea id="notes" name="notes" rows="3"
                                              class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white"
                                              placeholder="Any additional details about the request"></textarea>
                                </div>
                            </div>
...
function updateItemDetails() {
    ...
    if (selectedOption.value) {
        ...
        document.getElementById('item-available').textContent = available;
